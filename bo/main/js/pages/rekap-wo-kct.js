$(document).ready(function () {
    ...
    var table = $('#tbl_wo_kct')
      ...
      .DataTable( {
        ...
        "deferLoading": 1000,
        "ajax": {
            "url": '../controller/kct/getRekapWOKCT.php',
            "type": "POST",
            "timeout": 60000
        },
        //responsive: true,
        ...
        buttons: [
           // { extend: 'copyHtml5', text: '<i class="fa fa-clone"></i>&nbsp;&nbsp;Copy', footer: true },
            ...
            //       window.open('../controller/kct/expCSVDetailRPP.php?unitupi='+$('#sel_unitupi').val()+'&unitap='+$('#sel_unitap').val()+'&unitup='+$('#sel_unitup').val()+'&petugas='+$('#sel_petugas').val()+'&rpp='+$('#sel_rpp').val() );
            //   }

            // },
            // { 
            //   text: '<i class="fa fa-gear"></i>&nbsp;&nbsp;Ambil Rekomendasi', 
            //   titleAttr: 'Ambil Rekomendasi',
            //   //"sTooltip": "My CSV button's tooltip",
            //   action: function ( e, dt, node, config ) {
            //     console.log('dt',dt);
            //     var data;

            //     function getRekomendasiRPP(i){
            //       console.log('get rekomendasi ke '+i, dt.data());

            //       if(i<dt.data().length){

            //         data = dt.row( i ).data();
            //         dt.cell({row: i, column: 10}).data('Mohon menunggu...');  
            //         $.getJSON('../controller/kct/getRekomendasiRPP.php?unitup='+data.UNITUP+'&petugas='+data.USER_INPUT+'&latitude='+data.LATITUDE+'&longitude='+data.LONGITUDE, function(resp){
    ...
    $('#btn_cari').click(function(){
        ...
        table.ajax.url( '../controller/kct/getRekapWOKCT.php?unitupi='+$('#sel_unitupi').val()+'&unitap='+$('#sel_unitap').val()+'&unitup='+$('#sel_unitup').val()+'&petugas='+$('#sel_petugas').val() ).load();
    ...
    $(document).on("click", "a.cek-rekomendasi", function(e){
      e.preventDefault();
      ...
      var kddk = $(this).data('kddk');
      var latitude = $(this).data('latitude');
      var longitude = $(this).data('longitude');
      var idpel = $(this).data('idpel');

      $('div#map').block({ message: 'Mengambil peta RPP '+rpp+'...' });
      ...
      $.getJSON( "../controller/kct/getDataRPP.php", 
        { 
            unitupi: unitupi,
            unitap: unitap,
            unitup: unitup,
            ...
            if(json!=null){
                ...
                map = $('#map')
                  ...
                  // })
                    ...
                    //                                 $.post('../controller/kct/simpanRPP.php', 
