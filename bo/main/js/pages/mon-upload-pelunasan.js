$(document).ready(function () {
    ...
    //$.blockUI({ message: '<h1 class="p-3">Mengambil data...</h1>' }); 
    var table = $('#tbl_upload_pelunasan')
      .on('preXhr.dt', function ( e, settings, data ) {
        console.log('preXhr!');
        $('div.content-body').block({ message: 'Mengambil data...' });
      })
      ...
      .DataTable( {
        ...
        "deferLoading": 0,
        "ajax": {
            "url": '../controller/pelunasan/getUploadPelunasan.php',
            "type": "POST",
            "timeout": 60000
        },
        //responsive: true,
    ...
    $( 'body' ).on( 'click', '#btn_cari', function(btn) {
        ...
        table.ajax.url( '../controller/pelunasan/getUploadPelunasan.php?unitupi='+$('#sel_unitupi').val()+'&unitap='+$('#sel_unitap').val()+'&unitup='+$('#sel_unitup').val()+'&blth='+$('#sel_blth').val() ).load();
    });
