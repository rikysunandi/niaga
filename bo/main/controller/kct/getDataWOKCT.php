...
$sql = "EXEC sp_vw_Create_Data_WO_KCT @UserID = ?, @Unitupi = ?, @Unitap = ?, @Unitup = ?, @Petugas = ? ";
...
if($stmt){
	...
	while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
		...
		$response['plg'][$i]['petugas_priangan'] = ($row['petugas']); 
		$response['plg'][$i]['nama_gardu'] = ($row['gardu']); 
		$response['plg'][$i]['rbm_paska'] = ($row['vrpp']); 
