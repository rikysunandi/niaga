...
<script>
    ...
    function loadData(page) {
        ...
        for (var i = start; i < Math.min(end, data.length); i++) {
        	//console.log(data[i].idpel, data[i].aksi);
            var row = '<tr>' +
                '<td>' + data[i].idpel + '</td>' +
                '<td>' + data[i].nometer + '</td>' +
                '<td>' + data[i].merkmeter + '</td>' +
                '<td>' + data[i].petugas + '</td>' +
                '<td>' + data[i].tanggal + '</td>' +
                '<td><img id="img' + i + '" src="' + data[i].gambar + '" alt="Foto KCT Tidak ada">' +                
                '&nbsp;<button class="rotate-button" onclick="rotateImage(' + i + ')">Rotate</button>' +
                '</td>' +
                '<td>' +
                '<select onchange="updateAction(this, ' + i + ')">' +
                '<option value="Foto Tidak Sesuai"' + (data[i].aksi.trim() === 'Foto Tidak Sesuai' ? ' selected' : '') + '>Foto Tidak Sesuai</option>' +
                '<option value="Foto Sesuai"' + (data[i].aksi.trim() === 'Foto Sesuai' ? ' selected' : '') + '>Foto Sesuai</option>' +
                '</select>' +
                '</td>' +
                '</tr>';
    ...
    // Fungsi untuk mengupdate aksi pada data yang dipilih
    function updateAction(selectElement, index) {
        data[index].aksi = selectElement.value;
    }
    ...
    function exportData() {
        ...
        //csvContent += "IDPEL,Tanggal,Gambar,Aksi\n";
        data.forEach(function(item) {
            csvContent += item.idpel + "," + item.nometer + "," + item.merkmeter + "," + item.petugas + "," + item.tanggal + "," + item.gambar + "," + item.aksi + "\n";
        });
    ...
    function updateData() {
        var csvContent = "IDPEL,Tanggal,Gambar,Aksi\n";
        data.forEach(function(item) {
            csvContent += item.idpel + "," + item.nometer + "," + item.merkmeter + "," + item.petugas + "," + item.tanggal + "," + item.gambar + "," + item.aksi + "\n";
        });
        var encodedUri = encodeURI(csvContent);
