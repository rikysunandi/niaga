...
$authorization = "Bearer eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiI1My5NQU5BR0VSLEFHVU5HIE5VR1JBSEEiLCJpc3MiOiJORVdJVE8iLCJpYXQiOjE3MjkxNzE1ODEsImV4cCI6MTcyOTIwMDM4MX0.EnKnLT4JqJ1D2WPGljq-NWLPlAyAWowNlFq2yfyc6DGUcB2dVNWkdZBWIDsBITDWwbkaLkn78HbIuEEs5wu21g";

$context = stream_context_create(array(
    'http' => array(
        'header' => array(
        		"Authorization: " . $authorization,
        		"Origin: http://10.68.35.74:9000",
        		"Referer: http://10.68.35.74:9000/",
        		"Sec-Fetch-Mode: cors",
...
// $api_url = 'http://10.68.35.74:3000/api/newito/dtks/rekap?is_survey=SEMUA&tgl_survey_awal=01/10/2024&tgl_survey_akhir=09/10/2024&kode_petugas=SEMUA&level=UID&unit=53';
$api_url = 'http://10.68.35.74:3000/api/newito/dtks/rekap?is_survey=false&tgl_survey_awal=01/10/2024&tgl_survey_akhir='.date('d/m/Y').'&kode_petugas=SEMUA&level=UID&unit=53';
//echo $api_url;
...
$api_url = 'http://10.68.35.74:3000/api/newito/dtks/rekap?is_survey=true&tgl_survey_awal=01/10/2024&tgl_survey_akhir='.date('d/m/Y').'&kode_petugas=SEMUA&level=UID&unit=53';
//echo $api_url;
$result = file_get_contents($api_url, false, $context);

$data =  json_decode($result);
$waktu_notifikasi = str_replace('T',' ',substr($data->time,0,strlen($data->time)-10));
echo '<br/><br/>NON DTKS<hr/>';
echo json_encode($data->data);
...
if($data->message=='success'){
    ...
    if (count($data->data->content)) {
        ...
        foreach ($data->data->content as $idx => $row) {
            ...
            echo "<td>".time_elapsed_string('@'.(strtotime($waktu_notifikasi)), '@'.(strtotime($data->time) - $row->statusDateSecondAgo), true)."</td>";
