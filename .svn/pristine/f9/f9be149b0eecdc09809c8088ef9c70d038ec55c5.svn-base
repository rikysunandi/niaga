...
$stmt = sqlsrv_query($conn, "select * from vw_daftung_kondisi where noagenda_individu = '$noagenda' ");
if($stmt){
	...
	if($row['NOAGENDA_INDIVIDU']<>''){
		...
		$response['status_kelayakan'] = ($row['STATUS_KELAYAKAN']); 
		$response['keterangan_kelayakan'] = ($row['KETERANGAN_KELAYAKAN']); 
		$response['kendala_pln'] = ($row['KENDALA_PLN']); 
		$response['kendala_plg'] = ($row['KENDALA_PLG']); 
		$response['sudah_spb'] = ($row['SUDAH_SPB']); 
		$response['belum_spb'] = ($row['BELUM_SPB']); 
		$response['prosentase_pekerjaan'] = ($row['PROSENTASE_PEKERJAAN']); 
		$response['tgl_estimasi_nyala'] = date_format($row['TGL_ESTIMASI_NYALA'],'Y-m-d'); 
		$response['solusi_percepatan'] = ($row['SOLUSI_PERCEPATAN']); 
